...
func.func @omp_taskloop_missing_loop() -> () {
  // expected-error @below {{'omp.taskloop.context' op expected exactly one TaskloopWrapperOp in the region, but 0 were found}}
  omp.taskloop.context {
    omp.terminator
  }
  return
...
func.func @omp_taskloop_shared_context(%lb: index, %ub: index, %step: index) -> () {
  // expected-error @below {{'omp.taskloop.context' op expected exactly one TaskloopWrapperOp in the region, but 2 were found}}
  omp.taskloop.context {
    omp.taskloop.wrapper {
      omp.loop_nest (%i) : index = (%lb) to (%ub) step (%step)  {
        omp.yield
